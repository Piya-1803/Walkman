...
const SearchMusic = (props) => {
  ...
  const searchTracks = async () => {
    try {
      if (!authData) {
        // If authData is not available, wait for it before searching
        setTimeout(() => searchTracks(), 500);
        return;
      }
  ...
  // Add this function to handle adding a song to the playlist
  const handleAddToPlaylist = (track) => {
    const songDetails = {
      name: track.name,
      artist: track.artists.map((artist) => artist.name).join(', '),
      photoUrl: track.album.images[0].url,
    };
    props.addSongToPlaylist(songDetails);
  };

  // Modify the return section to include a click handler for each track
  return (
    <>
